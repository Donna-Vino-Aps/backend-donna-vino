...
export const validateUser = (userObject, isUpdate = false) => {
  const errorList = [];
  ...
  if (!isUpdate || userObject.firstName !== undefined) {
    if (!userObject.firstName || userObject.firstName.trim() === "") {
      errorList.push("First name is a required field.");
    } else if (!/^[a-zA-Z0-9]+(?: [a-zA-Z0-9]+)*$/.test(userObject.firstName)) {
      errorList.push(
  ...
  // Validate lastName
  if (!isUpdate || userObject.lastName !== undefined) {
    if (!userObject.lastName || userObject.lastName.trim() === "") {
      errorList.push("Last name is a required field.");
    } else if (!/^[a-zA-Z0-9]+(?: [a-zA-Z0-9]+)*$/.test(userObject.lastName)) {
      errorList.push(
  ...
  // Validate email
  if (!isUpdate || userObject.email !== undefined) {
  ...
  if (authProvider === "local") {
    // Validate password
    if (!isUpdate || userObject.password !== undefined) {
    ...
    if (!isUpdate || userObject.dateOfBirth !== undefined) {
